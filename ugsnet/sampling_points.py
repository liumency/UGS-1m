...
def point_sample(input, point_coords, **kwargs):
    ...
    if point_coords.dim() == 3:
        add_dim = True
        # add a dim at the axis=2
        point_coords = point_coords.unsqueeze(2)
    # print(f"in sampling point fun the size of input {input.shape} and point_coords {point_coords.shape}")
...
def sampling_points(mask, N, k=3, beta=0.75, training=True):
    ...
    # print(f"the point coord is {over_generation}")
    ...
    # print(f"the size of over_generation_map is {over_generation_map.shape}")
    ...
    # shift[:, None] increase the dim at the 1 channel
    idx += shift[:, None]
